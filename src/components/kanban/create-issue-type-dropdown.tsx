...
import { ISSUE_TYPES, ISSUE_TYPE_LABELS, type IssueType } from "@/constants/issue-type";
import { cn } from "@/lib/cn";

const OPTIONS: IssueType[] = [...ISSUE_TYPES];

export type CreateIssueTypeDropdownProps = {
  value: IssueType;
...
export function CreateIssueTypeDropdown({
  value,
  onChange,
  id,
  ...
  const handleSelect = (option: IssueType) => {
    onChange(option);
    close();
  ...
  return (
    <>
      <div ref={wrapRef} className={cn("w-full", className)}>
        ...
        >
          <span>{ISSUE_TYPE_LABELS[value]}</span>
          <ChevronDown
            className={cn(
          ...
          >
            {OPTIONS.map((opt) => {
              ...
              return (
                ...
                >
                  <span>{ISSUE_TYPE_LABELS[opt]}</span>
                  {isSelected && (
